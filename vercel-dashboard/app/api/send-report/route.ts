...
export async function POST() {
  // Gmail SMTP (앱 비밀번호) 발송
  const user = process.env.GMAIL_USER;
  const pass = (process.env.GMAIL_APP_PASSWORD || '').replace(/\s+/g, ''); // 앱비번 공백 제거
  const to = (process.env.REPORT_TO || '').split(',').map((s) => s.trim()).filter(Boolean);
  if (!user || !pass || !to.length) {
    return NextResponse.json(
      { ok: false, error: '환경변수(GMAIL_USER / GMAIL_APP_PASSWORD / REPORT_TO) 미설정' }, { status: 500 });
  }
  try {
    ...
    <div style="font-family:Malgun Gothic,Arial,sans-serif;max-width:640px;margin:0 auto;color:#1f2937">
      ...
      ${recon ? `
      <h3 style="margin:18px 0 6px">재접수율 (동일차량 3일내)</h3>
      <table style="width:100%;border-collapse:collapse;font-size:14px">
      ...
      <p style="color:#9ca3af;font-size:12px;margin-top:20px">대시보드에서 수동 발송 · PII 마스킹</p>
    </div>`;
